...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask("phantomizer-html-assets", "Builds assets within an html file", function () {
        ...
        var meta_path = options.meta_path
        ...
        if( meta_manager.is_fresh(meta_file) == false ){
            ...
            */
            ...
            ForEachNodeFileFound(lnodes, paths, function(n, node, in_file){
                ...
                queue_css_build(sub_tasks, current_target, out_path+osrc, meta_path+osrc+".meta", in_file, paths)

                if( image_merge ){
                    var tsrc = osrc.replace(file_suffix+".css", ".css")
                    tsrc = tsrc.replace(".css", "-im"+file_suffix+".css")
                    queue_css_img_merge( sub_tasks, current_target, out_path, meta_path, osrc, tsrc, paths )
                    osrc = tsrc
                }
            ...
            if( requirejs_src != false ){
                ...
                for( var nn in requirejs_src ){
                    ...
                    ForEachNodeFileFound(rscripts, paths, function(n, node, in_file){
                        ...
                        queue_requirejs_build(sub_tasks, current_target, out_path+osrc, meta_path+osrc+".meta", msrc)
            ...
            if( uglify_js ){
                ...
                ForEachNodeFileFound(scripts, paths, function(n, node, in_file){
                    if( in_file.indexOf("-min") == -1 &&  in_file.indexOf(".min") == -1 ){
                        ...
                        queue_uglifyjs_build( sub_tasks, current_target, out_path+tsrc, meta_path+tsrc+".meta", in_file )
            ...
            if( imgcompressor ){
                ...
                ForEachNodeFileFound(inodes, paths, function(n, node, in_file){
                    if( in_file.match("\.(png|jpeg|jpg)$") != null ){
                        ...
                        queue_img_opt(sub_tasks, current_target, out_path, meta_path, paths, in_file, node.asrc, osrc, options )
                        html_content = html_content.replace(node.node, node.node.replace(node.src, osrc) )
                    }
                })
            ...
            if( manifest == true ){
                ...
                queue_html_manifest( sub_tasks, current_target, out_path, meta_path, current_html_meta_file, current_html_out_file, in_request, current_html_in_file )
            ...
            if( htmlcompressor == true ){
                ...
                queue_html_min( sub_tasks, current_target, current_html_out_file, current_html_meta_file, current_html_in_file )
                current_html_in_file = current_html_out_file
                current_html_out_file = out_file
                grunt.log.ok("html minified")
            }

            queue_grunt_copy( sub_tasks, current_target, current_html_out_file, current_html_in_file )
    ...
    // -- task queue-er
    function queue_html_min( sub_tasks, current_target, out_file, meta_file, in_file ){
        ...
        task_options[jit_target].options.meta = meta_file
    ...
    function queue_img_opt( sub_tasks, current_target, out_path, meta_path, paths, in_file, asrc, osrc, options ){
        ...
        task_options[jit_target].options.meta_dir = meta_path
    ...
    function queue_uglifyjs_build( sub_tasks, current_target, out_file, meta_file, in_file ){
    ...
    function queue_css_img_merge( sub_tasks, current_target, out_dir, meta_dir, osrc, tsrc, paths ){
        ...
        task_options[jit_target].options.meta_file = meta_dir+tsrc+".meta"
        task_options[jit_target].options.paths = paths
    ...
    function queue_grunt_copy( sub_tasks, current_target, out_file, in_file ){
